...
Now that we have our datasets, we can load them and join them as a single dataset using the 'day' value. We can inspect some rows of our dataset using the *head()* function of Pandas.
...
Since as a non-american I don't really use Fahrenit, I'll  convert temperatures to Celusius. (the formula is $\frac{T-32}{1.8}$, if you're wondering)
